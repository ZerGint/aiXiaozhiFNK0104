...
void Backlight::RestoreBrightness() {
    ...
    if (saved_brightness < 10) {
        ESP_LOGW(TAG, "Brightness value (%d) is below minimum, setting to 10", saved_brightness);
        saved_brightness = 10;
    } else if (saved_brightness > 100) {
        saved_brightness = 100;
    }

    // Apply the restored value immediately so the hardware and slider agree.
