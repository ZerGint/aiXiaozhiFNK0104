...
#define TAG "FreenoveFnk0104sBoard"
...
class FreenoveFnk0104sBoard : public WifiBoard {
private:
    Button boot_button_;
    ...
    void InitializeTouch() {
        // Toggle FT6336 reset pin (GPIO 18) using ROM delays
        ...
        esp_rom_delay_us(10000); // 10ms ROM delay
        gpio_set_level(GPIO_NUM_18, 1);
        esp_rom_delay_us(50000); // 50ms ROM delay

        if (!touch_.Init(codec_i2c_bus_, 0x38)) {
            ESP_LOGE(TAG, "Failed to initialize FT6336 touch driver at 0x38");
            return;
        }

        ESP_LOGI(TAG, "Successfully registered FT6336 touch driver with LVGL!");
        lv_indev_t *indev = lv_indev_create();
        lv_indev_set_type(indev, LV_INDEV_TYPE_POINTER);
        lv_indev_set_read_cb(indev, [](lv_indev_t *indev, lv_indev_data_t *data) {
            auto self = static_cast<FreenoveFnk0104sBoard*>(lv_indev_get_user_data(indev));
            bool touched = false;
            uint16_t raw_x = 0, raw_y = 0;
            self->touch_.Read(touched, raw_x, raw_y);
    ...
    void InitializeLcdDisplay() {
        ...
        ESP_LOGD(TAG, "Install LCD driver ST7796");
    ...
    void InitializeBluetoothController() {
        ESP_LOGI(TAG, "Initializing early Bluetooth controller & NimBLE stack at boot...");
...
DECLARE_BOARD(FreenoveFnk0104sBoard);
