...
logfile_path = f'serial_log_{timestamp}.txt'
...
try:
    with open(logfile_path, 'w', encoding='utf-8', errors='replace') as logfile:
        logfile.write(f"--- CONTINUOUS LOGGING STARTED AT {time.strftime('%Y-%m-%d %H:%M:%S')} ---\n")
        logfile.flush()
